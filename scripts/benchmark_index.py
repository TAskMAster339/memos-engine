...
def main():
    ...
    parse_insert = r1.get("parse_insert_embed", 0)
    resolve_t = r1.get("resolve", 0)
    print(f"  parse+insert: {parse_insert:.2f}s")
    print(f"  resolve:      {resolve_t:.2f}s")

    # Phase 2: full index with embeddings
    print("=== Phase 2: index with embeddings ===")
    r2 = _run_index(repo)
    full_parse = r2.get("parse_insert_embed", 0)
    full_resolve = r2.get("resolve", 0)
    db_size = r2.get("db_size", 0)
    total = r2.get("total", 0)
    embed_time = full_parse - parse_insert if full_parse > parse_insert else 0
    print(f"  parse+insert+embed: {full_parse:.2f}s")
    print(f"  of which embed:     {embed_time:.2f}s")
    print(f"  resolve:            {full_resolve:.2f}s")
